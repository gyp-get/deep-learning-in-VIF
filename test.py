...
def main(ir_dir='./test_imgs/ir', vi_dir='./test_imgs/vi', save_dir='./SeAFusion', fusion_model_path='./model/Fusion/fusionmodel_final.pth'):
    ...
    with torch.no_grad():
        for it, (img_vis, img_ir, name) in enumerate(test_bar):
            ...
            # print('imgimgimgvis:',img_vis.shape,img_vis) # 1,3,480*640
            vi_Y, vi_Cb, vi_Cr = RGB2YCrCb(img_vis)
            # print('imgimgyyyyy:', vi_Y)
            vi_Y = vi_Y.to(device)
...
if __name__ == '__main__':
    ...
    ## dataset
    parser.add_argument('--ir_dir', '-ir_dir', type=str, default='./test_imgs/ir1')
    parser.add_argument('--vi_dir', '-vi_dir', type=str, default='./test_imgs/vis1')
